...
export default function EmailVerifizieren() {
  ...
  useEffect(() => {
    ...
    api.get<{ nachricht: string }>(`/auth/email-verifizieren/${token}`)
      .then((res) => { setStatus('erfolg'); setNachricht(res.nachricht); })
      .catch((err) => { setStatus('fehler'); setNachricht((err as Error).message); });
  }, [token]);

  return (
    <div className="min-h-dvh bg-gradient-to-br from-gray-50 via-red-50/30 to-orange-50/20 dark:from-[#0A0F1A] dark:via-[#0A0F1A] dark:to-[#0F1724] flex items-center justify-center p-4 font-body">
        ...
        <div className="bg-white/80 dark:bg-white/[0.04] backdrop-blur-sm rounded-2xl shadow-lg shadow-black/5 border border-white/50 dark:border-white/[0.07] p-6 text-center space-y-4">
          {status === 'laden' && (
            <>
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400">E-Mail wird verifiziert…</p>
            </>
          )}
          {status === 'erfolg' && (
            <>
