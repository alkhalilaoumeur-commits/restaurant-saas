...
export default function Einladung() {
  ...
  const [info, setInfo] = useState<EinladungInfo | null>(null);
  ...
  // Einladung prüfen beim Laden
  useEffect(() => {
    if (!token) return;
    api.get<EinladungInfo>(`/auth/einladung/${token}`)
      .then(setInfo)
  ...
  async function submit(e: FormEvent) {
    ...
    setAbsenden(true);
    try {
      const res = await api.post<EinladungResponse>('/auth/einladung-annehmen', {
        token,
        passwort,
        passwort_bestaetigung: passwortBestaetigung,
      });
      loginStore(res.token, res.mitarbeiter);
      navigate('/dashboard');
    } catch (err) {
  ...
  const rolleLabel = info?.rolle === 'admin' ? 'Administrator' : info?.rolle === 'kellner' ? 'Kellner' : 'Küche';
        ...
        <div className="bg-white/80 dark:bg-white/[0.04] backdrop-blur-sm rounded-2xl shadow-lg shadow-black/5 border border-white/50 dark:border-white/[0.07] p-6">
          ...
          {laden && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-3">Einladung wird geprüft…</p>
            </div>
          )}

          {/* Fehler (kein Info → ungültiger Link) */}
          {!laden && !info && (
            <div className="text-center py-6 space-y-4">
              ...
              <p className="text-sm text-gray-700 dark:text-slate-300">{fehler || 'Ungültiger Einladungslink'}</p>
              <Link to="/login" className="text-brand-primary text-sm font-medium hover:underline">Zum Login</Link>
            </div>
          )}
          ...
          {!laden && info && (
            <>
              <div className="mb-5 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  Hallo <strong>{info.name}</strong>! Du wurdest als <strong>{rolleLabel}</strong> zu <strong>{info.restaurantName}</strong> eingeladen.
                </p>
              </div>
              ...
              <form onSubmit={submit} className="space-y-4" noValidate>
                <div>
                  ...
                  <input type="email" value={info.email} disabled className={`${inputClass} opacity-60 cursor-not-allowed`} />
                </div>

                <div>
                  <label htmlFor="passwort" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Passwort festlegen</label>
