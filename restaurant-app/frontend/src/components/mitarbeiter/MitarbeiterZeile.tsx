...
export default function MitarbeiterZeile({ mitarbeiter, istEigenerAccount, onBearbeiten, onToggleAktiv, onErneutEinladen, onFotoAktualisieren }: MitarbeiterZeileProps) {
  ...
  return (
    <div className={`bg-white dark:bg-white/[0.04] dark:border dark:border-white/[0.07] rounded-xl p-4 shadow-sm card-hover ${!mitarbeiter.aktiv ? 'opacity-60' : ''}`}>
      {/* Zeile 1: Avatar + Info */}
      <div className="flex items-center gap-3 mb-2.5">
        ...
        {/* Rolle — rechts oben auf Desktop */}
        <span className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 hidden sm:inline-flex ${ROLLEN_FARBE[mitarbeiter.rolle]}`}>
          {ROLLEN_LABEL[mitarbeiter.rolle]}
        </span>
      </div>

      {/* Zeile 2: Rolle (mobile) + Datum + Aktionen */}
      <div className="flex flex-wrap items-center gap-1.5">
        {/* Rolle auf Mobile */}
        <span className={`text-xs px-2.5 py-1 rounded-full font-medium sm:hidden ${ROLLEN_FARBE[mitarbeiter.rolle]}`}>
          {ROLLEN_LABEL[mitarbeiter.rolle]}
        </span>

        {/* Datum */}
        <p className="text-xs text-gray-400 dark:text-slate-500 hidden sm:block mr-1">
          seit {formatDatum(mitarbeiter.erstellt_am).split(' ')[0]}
        </p>
